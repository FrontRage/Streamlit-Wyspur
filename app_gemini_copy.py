...
def filter_tool():
    ...
    if st.session_state.filter_step == 1:
        ...
        if st.button("Start Filtering", key="filter_start_filtering_button_step7"): # Added key
            ...
            if st.session_state.filter_apply_fuzzy and st.session_state.filter_column_config:
                df_pre_filtered, excluded_rows = python_pre_filter_fuzzy(df_for_filtering, st.session_state.filter_column_config, threshold=85)
...
def apply_saved_filter_to_ui(filter_params):
    ...
    # st.session_state.filter_column_keywords = filter_params.get("column_keywords", {}) # <--- REMOVE OLD LINE
...
def reset_filter_parameters():
    ...
    # st.session_state.filter_column_keywords = {} # <--- REMOVE OLD LINE
